...
bool loadFromFile(const std::string& file_path, GridGraph& graph)
{
    ...
    graph.collision_radius = ROBOT_RADIUS + graph.meters_per_cell;  // make collision radius larger when testing robot
...
{
    graph.nodes.clear();
    for (int i = 0; i < graph.width * graph.height; ++i)   // create a node for each cell
    {
        Node node;
        node.visited = false;
        node.queued = false;
        node.parent = -1;  // No parent
        node.cost = HIGH;  
        // node.score = HIGH; 
...
std::vector<int> findNeighbors(int idx, const GridGraph& graph)
{
     ...
     * bounds of the graph.
     * 
     * should return 4 neighbors (up, down, left, right) if all are in bounds
     *
     * HINT: The functions idxToCell(), cellToIdx(), and isCellInBounds() might
     * come in handy.
     */
    Cell currentCell = idxToCell(idx, graph);  // (i, j)
    ...
    for (int i = 0; i < directions.size(); ++i) {
        int neighbor_i = currentCell.i + directions[i].first;
        int neighbor_j = currentCell.j + directions[i].second;

        // only return if in bounds
        if (isCellInBounds(neighbor_i, neighbor_j, graph)) {
            neighbors.push_back(cellToIdx(neighbor_i, neighbor_j, graph));
        }
    }
