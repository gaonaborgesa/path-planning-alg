...
{
    std::vector<Cell> path;      // The final path should be placed here.
    std::queue<int> visit_queue; // index queue for BFS
    ...
    graph.nodes[start_idx].cost = 0;      // zero cost
    visit_queue.push(start_idx);          // add start to queue
    graph.nodes[start_idx].queued = true; // set queued to true

    // if queue is empty then goal not found
    while (!visit_queue.empty())
    {
        int current_idx = visit_queue.front();   // get index of first node in queue
        visit_queue.pop();                       // dequeue the front node
        graph.nodes[current_idx].visited = true; // remove?
        graph.visited_cells.push_back(idxToCell(current_idx, graph));    // for visualization
        ...
        for (int neighbor : findNeighbors(current_idx, graph)) // returns vector of 4 neighbor indices
        {
            if (!graph.nodes[neighbor].visited && !graph.nodes[neighbor].queued && !checkCollision(neighbor, graph))
            {
                graph.nodes[neighbor].parent = current_idx;
                graph.nodes[neighbor].cost = graph.nodes[current_idx].cost + 1;
                ...
                // if (graph.nodes[neighbor].cost > graph.nodes[current_idx].cost + 1)
                // {
                //     // set parent and cost, mark queued and push to visit queue
                //     graph.nodes[neighbor].cost = graph.nodes[current_idx].cost + 1;
                // }
            }
        }
    ...
    // If we exit the loop without findings the goal, no path was found.
    std::cout << "No path found :(\n";
    return path; // if no path found
}
