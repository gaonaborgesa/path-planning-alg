...
struct Node
{
    bool visited;
    bool queued;
    int parent;  // index of parent in graph data
    float cost;  // cost to reach cell
    // float score; // for A* - f-score for node
};
...
struct GridGraph
{
    GridGraph() : width(-1),
                  ...
                  threshold(-100) // TODO: Adjust threshold.     (maybe?)
    {
    };
    ...
    std::vector<Node> nodes;  // nodes in cell graph
};
